        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="panel panel-default">
                    <div class="panel-heading">Priceregistrationdatail</div>
                            ...
                            <table class="table table-borderless">
                                ...
                                @foreach($priceregistrationdatail as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url('/price-registration-datail/' . $item->id) }}" title="View PriceRegistrationDatail"><button class="btn btn-info btn-xs"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                                            <a href="{{ url('/price-registration-datail/' . $item->id . '/edit') }}" title="Edit PriceRegistrationDatail"><button class="btn btn-primary btn-xs"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

                                            <form method="POST" action="{{ url('/price-registration-datail' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="btn btn-danger btn-xs" title="Delete PriceRegistrationDatail" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
